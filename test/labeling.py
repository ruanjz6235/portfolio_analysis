...
# %%
def get_hv_return_label(request_id, type):
    ...
    for m, i in enumerate(groups):
        ...
        conn = cx.connect('zdj', 'xtKFE8k3ctqbYDOz', '10.55.57.53:1521/fundrate', encoding='UTF-8')
        cursor = conn.cursor()
        query = """select client_id, init_date, daily_return from hv_return where client_id in ({clients})""".format(
            clients=str(list(i))[1:-1])
        ...
        and TradingDay >= to_date('{start}', 'yyyy-mm-dd') and TradingDay <= to_date('{end}', 'yyyy-mm-dd') order by TradingDay asc
        """.format(start='2018-12-25', end=today.strftime('%Y-%m-%d'))
        ...
        clients_data = clients_data[[all_list[2], 'EndDate'] + list(all_list[3:])]
        for ll in clients_data.columns[2:44]:
        ...
        for ll in clients_data.columns[44:]:
            clients_data[ll] = clients_data[ll].apply(lambda x: float(x))
        values = list(tuple(clients_data.loc[ii]) for ii in clients_data.index)
        conn = cx.connect('zdj', 'xtKFE8k3ctqbYDOz', '10.55.57.53:1521/fundrate', encoding='UTF-8')
        cursor = conn.cursor()
        cursor.execute(f'delete from zs_client_rank_tertile where client_id in ({str(list(i))[1:-1]})')
        conn.commit()
        cursor.executemany(
            """insert into zs_client_rank_tertile (client_id, EndDate, annureturn_1y, annureturn_cy, annureturn_3y,
